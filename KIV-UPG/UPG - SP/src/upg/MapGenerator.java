...
public class MapGenerator {
     ...
     * @param w šířka
     * @param h výška
     * @param maxHeight maximální nadmořská výška
     * @return vygenerovaná mapa
     */
    public static Map generateMap(int w, int h, int maxHeight) {
        int bounds = 15;  // minimální vzdálenost od okraje
        ...
        return new Map() {{
            width = w;
            height = h;
            playerX = random.nextInt(w - 2*bounds) + bounds;
            playerY = random.nextInt(h - 2*bounds) + bounds;
            targetX = random.nextInt(w - 2*bounds) + bounds;
            targetY = random.nextInt(h - 2*bounds) + bounds;
            heightMap = generateHeightmap(w, h, maxHeight);
        }};
    }
    ...
    public static int[][] generateHeightmap(int w, int h, int maxHeight) {
        ...
        for (int i = 0; i < w; i++)
            for (int j = 0; j < h; j++)
                heightmap[j][i] = (int) (maxHeight * perlinNoise[i][j]);
